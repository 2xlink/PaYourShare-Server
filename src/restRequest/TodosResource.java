...
public class TodosResource {
	...
	public void deleteTodo(@FormParam("id") String id) {
		System.out.println("Attemption to remove " + id);
		TodoDao.instance.getModel().remove(id);
		System.out.println("Removed.");

		System.out.println(TodoDao.instance.getModel().toString());
	}
	...
	public TodoResource getTodo(@PathParam("todo") String id) {
		// try {
		System.out.println(uriInfo.getPath() + "," + request.getMethod());
		return new TodoResource(uriInfo, request, id);
		// }
		// catch (Exception e) {
		//// System.console().writer().write(e.getMessage());
		// return new TodoResource(uriInfo, request, "1");
		// }
	}
